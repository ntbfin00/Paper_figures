...
def plot_HOD_contribution(fn=None):

    params = ['omega_cdm', 'h', 'logA', 'n_s']
    tracers = ['LRG', 'ELG']
    
    # set line defaults
    line1_kwargs = {'ls': '--', 'color': 'k', 'label': 'V1'}
    line2_kwargs = {'ls': '-', 'color': 'k', 'label': 'V1+HOD ($P(k)$ level)'}
    ...
    for (z, tracer) in enumerate(tracers):
        ...
        density_nosys = MCSamplesCompressed.load(f"contour_{tracer}_nosys.npy")
        density_sys = MCSamplesCompressed.load(f"contour_{tracer}_sys.npy")
        ...
        g.plots_1d([density_nosys, density_sys], 
        ...
        # Parameter level PWE contribution (BGS assumed equal to LRG as cannot be calculated)
        MAP_mean = np.load('HODsys_MAPY1_{}.npy'.format('LRG' if tracer=='BGS' else tracer))
        ML_mean = np.load('HODsys_MLY1_{}.npy'.format('LRG' if tracer=='BGS' else tracer))
        diag_std = abs(MAP_mean - ML_mean).max(axis=-1)
        ...
        for (p, param) in enumerate(params):
            ...
            comb_std = np.sqrt(mean_std_nosys[z,p,1]**2 + HOD_std**2 + diag_std[p]**2) 
...
# plot subplots 
plot_HOD_contribution(fn=f'Figure3_{{}}.pdf')