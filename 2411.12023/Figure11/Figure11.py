...
def plot_wCDM_HOD_contribution(fn=None):

    params = ['omega_cdm', 'h', 'logA', 'n_s', 'w0_fld']
    tracers = ['LRG', 'ELG']
    ...
    for (z, tracer) in enumerate(tracers):
        ...
        for (p, param) in enumerate(params):
            ...
            if z<1: 
                xlim[p] = ax.get_xlim()
                xlim[p] += shiftx[p] * xlim[p].mean()  # shift limits by shiftx
...
# plot subplots
plot_wCDM_HOD_contribution(fn=f'Figure11_{{}}.pdf')